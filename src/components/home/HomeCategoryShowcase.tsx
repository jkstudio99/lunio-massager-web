      ...
      <div className="mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 sm:gap-6">
          {categories.map((category, i) => {
            const data = t.categoryData[category.id as keyof typeof t.categoryData];
            return (
              <motion.div
                key={category.id}
                {...fadeInUp}
              ...
              >
                ...
                >
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-6 sm:p-7">
                    ...
                    <h3 className="text-2xl font-bold text-white mb-2">{data?.name ?? category.name}</h3>
                    <p className="text-sm text-white/75 leading-relaxed max-w-xs">{data?.description ?? category.description}</p>
